...
################################################################################
class RabbitSensorAdapter(object):
    ...
    def initiateConsuming(self, timeout):
        '''Enters the pika IO loop to process events; Exits after /timeout/ seconds'''
        if timeout > 0.0:
            self.timeoutHandle = self.conn.add_timeout(timeout, self.blockerTimeout)
        else:
...
################################################################################
class RabbitCloudAdapter(object):
    ...
    def sendStreamItem(self, theString):
        if (self.myTxid > 0) & (self.shutdownRequested == False):
                msg = "%s %s" % (self.myTxid, theString)
                self.chan.basic_publish(exchange='streams',routing_key=self.myRoutekey,body=msg)
        else:
                print "You can't send a message, you have no valid tx id or shutdoQueueprogress."
